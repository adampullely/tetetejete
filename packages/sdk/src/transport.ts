// Base HTTP transport for the tetetejete SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `tetetejeteError`
...
export interface ClientOptions {
  /**
   * Base URL of the tetetejete api-edge worker, e.g.
   * `https://api.tetetejete.com`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `tetetejete` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "tetetejete SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "tetetejete SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
