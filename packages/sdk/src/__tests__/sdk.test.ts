...
import { tetetejete } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new tetetejete({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => tetetejeteError]> = [
  ...
  it("forward-compat: unknown error code → base tetetejeteError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(tetetejeteError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as tetetejeteError).code).toBe("future_quota_exceeded");
    }
  });
