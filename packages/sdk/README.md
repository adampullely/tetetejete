...
tetetejete TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the tetetejete control plane API.

## Install (workspace)

This package is internal to the `tetetejete` monorepo. Add it as a
...
```ts
import { tetetejete } from "@saas/sdk";

const client = new tetetejete({
  baseUrl: "https://api.tetetejete.com",
  auth: { kind: "bearer", token: process.env["TETETEJETE_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `tetetejeteError`. Branch on
...
| `tetetejeteError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `tetetejeteError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new tetetejete({ fetch })`.
