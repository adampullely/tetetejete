...
`tetetejete` — first-class TypeScript CLI for the tetetejete control
...
This package is internal to the `tetetejete` monorepo and is not
...
```
tetetejete login    [--api-url=URL] [--token=BEARER]
tetetejete logout
tetetejete whoami
```
...
```
tetetejete org list
tetetejete org use <org-id>
tetetejete org members
tetetejete project list
```

Writes (Task 0101):

```
tetetejete org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
tetetejete project create <name> [--idempotency-key=KEY]
tetetejete env create <project-id> <name> [--idempotency-key=KEY]
tetetejete api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
tetetejete webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
tetetejete usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
tetetejete billing summary
tetetejete audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`tetetejete org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `tetetejete login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/tetetejete/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/tetetejete/config.json` (mode 0644, not a secret). Override
both via `TETETEJETE_CONFIG_DIR` (used by tests).
