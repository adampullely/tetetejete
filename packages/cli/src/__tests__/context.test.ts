...
describe("resolveConfigDir", () => {
  it("respects TETETEJETE_CONFIG_DIR override", () => {
    const prev = process.env["TETETEJETE_CONFIG_DIR"];
    process.env["TETETEJETE_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["TETETEJETE_CONFIG_DIR"];
      else process.env["TETETEJETE_CONFIG_DIR"] = prev;
    }
  });
});
