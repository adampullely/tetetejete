// CLI error helpers. Translate `tetetejeteError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `tetetejete org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `tetetejete login` to refresh", mode);
  }
  if (err instanceof tetetejeteError) {
